...
#
# Usage:
#   ./scripts/ralph-loop-opencode.sh                         # Build mode (unlimited)
#   ./scripts/ralph-loop-opencode.sh 20                      # Build mode (max 20 iterations)
#   ./scripts/ralph-loop-opencode.sh plan                    # Planning mode (creates IMPLEMENTATION_PLAN.md)
#   ./scripts/ralph-loop-opencode.sh --model "provider/model" # Override model
#
# Requirements:
#   - OpenCode CLI installed: https://opencode.ai
#   - Run 'opencode' once interactively or 'opencode auth login' to authenticate.
#
...
OPENCODE_MODEL="${OPENCODE_MODEL:-}"  # Empty = use config default
...
Usage:
  ./scripts/ralph-loop-opencode.sh                            # Build mode, unlimited iterations
  ./scripts/ralph-loop-opencode.sh 20                         # Build mode, max 20 iterations
  ./scripts/ralph-loop-opencode.sh plan                       # Planning mode (optional)
  ./scripts/ralph-loop-opencode.sh --model "provider/model"   # Override model
...
Models:
  Use provider/model format, e.g.:
    OPENCODE_MODEL=anthropic/claude-sonnet-4-7 ./scripts/ralph-loop-opencode.sh
    ./scripts/ralph-loop-opencode.sh --model opencode-go/deepseek-v4-pro
  Run 'opencode models' to see all available models.
  Leave empty to use the default model from your opencode config.

How it works:
  1. Each iteration runs 'opencode run' with the build/plan prompt
  2. OpenCode picks the HIGHEST PRIORITY incomplete spec/task
  3. OpenCode implements, tests, and verifies acceptance criteria
  4. OpenCode outputs <promise>DONE</promise> ONLY if criteria are met
    ...
    case "$1" in
        ...
        --model)
            if [[ -n "${2:-}" ]]; then
                ...
                echo "Available models: run 'opencode models' to see the list"
                exit 1
            fi
            ;;
        -h|--help)
...
SESSION_LOG="$LOG_DIR/ralph_opencode_${MODE}_session_$(date '+%Y%m%d_%H%M%S').log"
exec > >(tee -a "$SESSION_LOG") 2>&1

# Check if OpenCode CLI is available
if ! command -v "$OPENCODE_CMD" &> /dev/null; then
    echo -e "${RED}Error: OpenCode CLI (opencode) not found${NC}"
    echo ""
    echo "Install OpenCode:"
    echo "  curl -fsSL https://opencode.ai/install | bash"
    echo "  npm install -g opencode-ai"
    echo ""
    echo "Then run 'opencode' once interactively or 'opencode auth login' to authenticate."
    exit 1
fi
...
# --model sets the model (omitted when OPENCODE_MODEL is empty to use config default)
OPENCODE_MODEL_FLAG=""
if [ -n "$OPENCODE_MODEL" ]; then
    OPENCODE_MODEL_FLAG="--model $OPENCODE_MODEL"
fi
...
echo ""
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}            RALPH LOOP (OpenCode CLI) STARTING                  ${NC}"
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
echo -e "${BLUE}Mode:${NC}     $MODE"
echo -e "${BLUE}Model:${NC}    ${OPENCODE_MODEL:-default (from config)}"
...
while true; do
    ...
    LOG_FILE="$LOG_DIR/ralph_opencode_${MODE}_iter_${ITERATION}_$(date '+%Y%m%d_%H%M%S').log"
    : > "$LOG_FILE"
    WATCH_PID=""

    if [ "$ROLLING_OUTPUT_INTERVAL" -gt 0 ] && [ "$ROLLING_OUTPUT_LINES" -gt 0 ] && [ -t 1 ] && [ -w /dev/tty ]; then
        watch_latest_output "$LOG_FILE" "OpenCode" &
        WATCH_PID=$!
    fi

    # Run OpenCode CLI with prompt passed as message argument
    # opencode run [message] runs non-interactively
    # --dangerously-skip-permissions auto-approves all tool calls
    # --model sets the model (omitted when empty to use config default)
    OPENCODE_OUTPUT=""
    OPENCODE_PROMPT=$(cat "$PROMPT_FILE")
    OPENCODE_FLAGS=""
    ...
    if [ "$YOLO_ENABLED" = true ]; then
        OPENCODE_FLAGS="$OPENCODE_FLAGS $YOLO_FLAG"
    fi

    if OPENCODE_OUTPUT=$("$OPENCODE_CMD" run "$OPENCODE_PROMPT" $OPENCODE_FLAGS 2>&1 | tee "$LOG_FILE"); then
        if [ -n "$WATCH_PID" ]; then
            kill "$WATCH_PID" 2>/dev/null || true
            wait "$WATCH_PID" 2>/dev/null || true
        fi
        echo ""
        echo -e "${GREEN}✓ OpenCode execution completed${NC}"

        # Check if DONE promise was output (accept both DONE and ALL_DONE variants)
        if echo "$OPENCODE_OUTPUT" | grep -qE "<promise>(ALL_)?DONE</promise>"; then
            DETECTED_SIGNAL=$(echo "$OPENCODE_OUTPUT" | grep -oE "<promise>(ALL_)?DONE</promise>" | tail -1)
            echo -e "${GREEN}✓ Completion signal detected: ${DETECTED_SIGNAL}${NC}"
            echo -e "${GREEN}✓ Task completed successfully!${NC}"
            CONSECUTIVE_FAILURES=0

            # Break if ALL_DONE
            if [ "$DETECTED_SIGNAL" = "<promise>ALL_DONE</promise>" ]; then
                echo ""
                echo -e "${GREEN}All work is done! Exiting loop.${NC}"
                break
            fi
            ...
            if [ "$MODE" = "plan" ]; then
                ...
                echo -e "${CYAN}Run './scripts/ralph-loop-opencode.sh' to start building.${NC}"
            ...
            print_latest_output "$LOG_FILE" "OpenCode"
...
echo ""
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}  RALPH LOOP (OpenCode) FINISHED ($ITERATION iterations)       ${NC}"
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
